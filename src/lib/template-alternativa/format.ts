// Helpers locais de formatação — isolados do restante do app para manter o
// protótipo autocontido (não reaproveita src/lib/utils.ts de propósito).
...
export function formatPhone(phone: string): string {
  ...
  if (nums.length > 10) return nums.replace(/(\d{2})(\d{1})(\d{4})(\d{4})/, '($1) $2 $3-$4')
