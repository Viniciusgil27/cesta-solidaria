    ...
    <div className="bg-white border border-[var(--tpl-border)] rounded-xl p-3 flex flex-col items-center gap-2">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="w-8 h-8 rounded-lg border border-[var(--tpl-border)] flex items-center justify-center text-[var(--tpl-primary)] font-bold hover:bg-[var(--tpl-primary-soft)] transition-colors"
        >
          −
        </button>
...
// Formulário 100% visual: valida no cliente e simula o envio do comprovante
// (preview local via URL.createObjectURL), sem chamar nenhuma API real.
export default function TemplateCadastroPage() {
  const router = useRouter()
  const fileInputRef = useRef<HTMLInputElement>(null)

  const [loading, setLoading] = useState(false)
  const [erro, setErro] = useState('')
  const [form, setForm] = useState({ nome: '', cpf: '', telefone: '', endereco: '', bairro: '', criancas: 0, adolescentes: 0, adultos: 0, idosos: 0 })
  ...
  function handleArquivo(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setTimeout(() => setUploadStatus('done'), 900)
    e.target.value = ''
  }
  ...
  function enviar(e: React.FormEvent) {
    ...
    if (uploadStatus !== 'done') { setErro('Envie o comprovante de residência para continuar.'); return }

    setLoading(true)
    setTimeout(() => {
      setLoading(false)
      router.push('/template-visual/cadastro/sucesso')
    }, 500)
  }
